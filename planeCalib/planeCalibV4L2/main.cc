/*
 *  $Id: main.cc 2155 2017-01-05 05:24:02Z ueshiba $  
 */
/*!
  \mainpage	planeCalibGUI - 既知の平面パターンを1台もしくはそれ以上のV4L2カメラに提示することによってカメラの内部・外部パラメータを推定するプロブラム
  ...
  \section abstract 概要
  planeCalibGUIは，既知の平面パターンを1台もしくはそれ以上のV4L2カ
...
static const double		DEFAULT_PITCH = 10.0;
...
usage(const char* s)
{
    ...
    cerr << "\nCalibrate V4L2-based multi-camera system with a planar pattern.\n"
	 << endl;
...
main(int argc, char* argv[])
{
  ...
  // Parse command options.
    extern char*	optarg;
    for (int c; (c = getopt(argc, argv, "c:RC:S:D:W:A:eL:H:p:h")) != -1; )
	switch (c)
	{
	  case 'c':
	    cameraName = optarg;
